...
test("runs a future forecast with mocked open-data results", async ({ page }) => {
  ...
  const mapLayer = page.getByLabel("Competitor map layer");
  await expect(mapLayer).toHaveValue("dots");
  await mapLayer.selectOption("heatmap");
  await expect(mapLayer).toHaveValue("heatmap");
});
